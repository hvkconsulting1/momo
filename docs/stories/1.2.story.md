...
- [ ] Install norgatedata package (AC: 1)
  ...
  - [ ] Run `uv sync` to install the package (may fail on WSL - this is expected)
  - [ ] Verify Windows Python has norgatedata installed (required for bridge)
  - [ ] Verify bridge can import and execute norgatedata code via `python.exe`
