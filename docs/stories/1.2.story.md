...
- [ ] Implement Windows Python bridge module (AC: 2)
  - [ ] Create `src/momo/data/bridge.py` with subprocess-based communication
  - [ ] Implement `execute_norgate_code()` function to run Python code via `python.exe`
  - [ ] Add JSON-based result serialization for data transfer between Windows and WSL Python
  - [ ] Implement logging using structlog for all bridge operations
...
- [ ] Implement error handling (AC: 4)
  - [ ] Create custom exception classes in `src/momo/utils/exceptions.py` for bridge errors
  - [ ] Handle NDU not running error (clear message to user)
  - [ ] Handle Windows Python not found error
  - [ ] Handle norgatedata package import errors
  - [ ] Handle JSON serialization/deserialization errors
  - [ ] Add timeout for subprocess calls (default 30 seconds)
...
- [ ] Write unit tests (AC: 6)
  ...
  - [ ] Write unit test for `execute_norgate_code()` with mock subprocess
  - [ ] Write unit test for `fetch_price_data()` with mock responses
  - [ ] Write unit test for error handling paths
